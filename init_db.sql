...
CREATE TABLE thermal_sessions(
    ...
    person_id INT,
    session_date DATE,
    room_temperature FLOAT CHECK(room_temperature BETWEEN -50.0 AND 50.0),
...
CREATE TABLE vital_signs(
    ...
    oxygen_saturation FLOAT CHECK(oxygen_saturation BETWEEN 0 AND 100),
    systolic_pressure FLOAT CHECK(systolic_pressure > 0),
    diastolic_pressure FLOAT CHECK(diastolic_pressure > 0),
    version VARCHAR(255) NOT NULL DEFAULT '1.0',
    version_timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (person_id) REFERENCES persons(person_id) ON DELETE CASCADE
);
